...
session.setAttribute("price",k);
       ...
       Connection con=databasecon.getconnection();
        ...
        String sql="insert into oneway1(passname,name,flightid,class,start,arrival,depar,adult,child,infant,price) values('"+a+"','"+b+"','"+c+"','"+d+"','"+e+"','"+f+"','"+g+"','"+h+"','"+i+"','"+j+"','"+k+"')";
		
         int x=st.executeUpdate(sql);
		if(x!=0){
		
		response.sendRedirect("makepayment.jsp?Successfully Booked");
		}
				else{
			response.sendRedirect("makepayment.jsp?message=fail");
		}
